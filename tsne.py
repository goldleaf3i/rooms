...
class TSNE(object) :
    # TODO CHECK E COMMENTARE # 
    def __init__(self,similarity_matrix_name = "similarity_matrix.txt", cluster_labels_name ="cluster_labels.txt",delimiter = ',' ):
        print("tsne performs symmetric t-SNE on affinity matrix P");
        self.P = Math.loadtxt(similarity_matrix_name,delimiter=delimiter);
        self.labels = Math.loadtxt(cluster_labels_name,delimiter=delimiter);
        self.Y = tsne(self.P);
        plotting();


    def tsne(self, P = Math.array([])):
